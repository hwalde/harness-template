...
4. For each agent enter **one sentence** in `AGENTS.md` under "Coding agents in this project": what it supports, where the files live, what is generated for it. Mark the uncertain as "(unconfirmed)".
...
This table goes stale on its own – the setup verifies every needed cell and records the result as one sentence each in `AGENTS.md`.
