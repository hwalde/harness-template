...
A skill can keep a `memory.md`/learnings file: read it at the start, extend it at the end. The phrasing is delicate ("with the **relevant** learnings", not "with all"); risk: the agent optimizes core decisions away. Countermeasure: mark non-negotiable decisions in the skill. For the knowledge store, the librarian takes over the curation ([wissensablage.md](wissensablage.md)).
