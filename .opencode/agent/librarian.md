...
description: >-
  ...
  sitzungsübergreifendem Wert: eine Entscheidung samt Begründung, eine stabile Erkenntnis /
  ein Muster / eine Konvention, ein hart erkämpfter Fallstrick, Betriebs-/Zugangswissen,
  Domänen-/Geschäftswissen; (c) WARTUNG (Konsistenzprüfung) nur auf expliziten Auftrag.
  NICHT für transientes Laufzeit-Wissen (eine gerade beantwortete Nachfrage,
  Fortschritts-/Statusnotizen, per grep aus dem Code wiederbeschaffbares Detail, Triviales)
  – so etwas lehnt der librarian ab. Er gibt nur ein knappes Destillat zurück, nie Rohdaten.
mode: subagent
tools:
  read: true
  grep: true
